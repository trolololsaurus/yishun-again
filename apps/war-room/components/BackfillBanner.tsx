  ...
  return (
    <div className="mb-6 border border-border bg-surface p-4">
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <span className="font-body text-text-secondary" style={{ fontSize: '10px' }}>
          {runDate}
        </span>
      </div>

      {/* Stats row */}
      <div className="flex flex-wrap gap-8 font-body mb-4" style={{ fontSize: '11px' }}>
        <div>
          <div className="text-text-secondary mb-0.5" style={{ fontSize: '9px' }}>SCRAPED</div>
          <div className="text-text-primary font-bold">{String(stats.scraped ?? '—')}</div>
        </div>
        <div>
          <div className="text-text-secondary mb-0.5" style={{ fontSize: '9px' }}>AUTO-PUBLISHED</div>
          <div className="text-green font-bold">{String(stats.auto_published ?? '—')}</div>
        </div>
        <div>
          <div className="text-text-secondary mb-0.5" style={{ fontSize: '9px' }}>QUEUED FOR REVIEW</div>
          <div className="text-yellow font-bold">{String(stats.queued_for_review ?? '—')}</div>
        </div>
        <div>
          <div className="text-text-secondary mb-0.5" style={{ fontSize: '9px' }}>UPDATES FOUND</div>
          <div className="text-text-primary font-bold">{String(stats.updates_found ?? '—')}</div>
        </div>
        <div>
          <div className="text-text-secondary mb-0.5" style={{ fontSize: '9px' }}>REJECTED</div>
          <div className="text-text-secondary font-bold">{String(stats.rejected ?? '—')}</div>
        </div>
        {(stats.errors as number) > 0 && (
          <div>
            <div className="text-text-secondary mb-0.5" style={{ fontSize: '9px' }}>ERRORS</div>
      ...
      {(highConfCount > 0 || lowConfCount > 0) && (
        <div className="border-t border-border pt-3 flex flex-wrap gap-3 items-center">
          ...
          {highConfCount > 0 && (
              ...
              <span className="font-body text-green" style={{ fontSize: '10px' }}>
                ✓ {done.high} approved
              </span>
            ) : (
              <button
          ...
          {error && (
            <span className="font-body text-red" style={{ fontSize: '10px' }}>
              ✗ {error}
            </span>
          )}
        </div>
