...
├── infra/
│   ├── fly.toml                # Fly.io config for agents backend
...
| Map | Mapbox GL JS | 3.x | Custom pixel art marker icons |
...
| Agent hosting | Fly.io | — | Single shared-cpu-1x machine to start |
...
CREATE TABLE incidents (
  ...
  classification  TEXT NOT NULL CHECK (classification IN ('heart', 'jester', 'dagger', 'custom')),
...
Given source content, return JSON only:
{
  ...
  "classification": "heart" | "jester" | "dagger",
  ...
  "tags": string[],
  "confidence": float (0.0-1.0),
  "chaos_contribution": float (1-5 scale, Daggers weighted 3x, Jesters 1.5x, Hearts -1x)
}

Classification guide:
- heart: Good news, community wins, positive stories
- jester: Absurd, stupid, baffling behaviour — no serious harm
...
URL format: `https://assets.yishunagain.sg/cards/[incident-id].png`

UTM share URL embedded in OG meta:
```html
<meta property="og:url" content="https://yishunagain.sg/incidents/[slug]?utm_source=share_card&utm_medium=og&utm_campaign=[classification]" />
```
...
def compute_chaos_index(window_days: int = 30) -> float:
    ...
    for inc in incidents:
        ...
        elif inc.classification == 'jester':
...
```
https://yishunagain.sg/incidents/{slug}?utm_source={source}&utm_medium=share_card&utm_campaign={classification}
```
...
# App
NEXT_PUBLIC_SITE_URL=https://yishunagain.sg
WAR_ROOM_URL=https://warroom.yishunagain.sg   # private subdomain, CF Access protected
```
...
# Domain: yishunagain.sg + www.yishunagain.sg
# Cloudflare proxy: YES (orange cloud on both)
```

### 11.2 Agents Backend (Fly.io)

```bash
# packages/agents
fly launch --name yishun-agents --region sin  # Singapore region
fly scale vm shared-cpu-1x --memory 512       # Start minimal
fly deploy
...
2. Create R2 bucket: `yishun-assets` → public access → custom domain `assets.yishunagain.sg`
3. Cloudflare Access: protect `warroom.yishunagain.sg` with email OTP (operator email only)
...
```bash
# apps/war-room
# Deploy to warroom.yishunagain.sg
...
<meta property="og:url" content="https://yishunagain.sg/incidents/{slug}" />
...
Step 3:  FastAPI skeleton on Fly.io — health check endpoint only
