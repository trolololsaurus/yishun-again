...
Step 16: ⏳ Historical backfill + hero incidents
         ✅ Hero incidents SQL (migration 005) — 8 incidents inserted
         ✅ Backfill agent built with dedup, link validator, geocoding
         ⏳ Live backfill runs (Day 1–3, 500 items/day)
...
| `revalidate = 0` on homepage | Medium | Must fix before prod | Change to 60 in apps/web/app/page.tsx |
...
| Wikipedia scraper untested live | Medium | Pending | Dry run never reached it (Groq quota) |
| Backfill intra-batch dedup untested live | Medium | Pending | Built, needs live confirmation |
| REVALIDATE_SECRET not in Vercel | Medium | Before launch | Add to Vercel env vars |
| Map tiles style clashes with dark palette | Medium | Tomorrow | Replacing map entirely |
| Groq free tier (500k TPD) | Medium | Backfill workaround | Split runs across days |
