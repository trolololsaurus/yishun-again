"""
MustShareNews scraper — RSS feed (spec: msm, 60-minute interval).
"""
...
SOURCE_NAME = "MustShareNews"
SOURCE_TYPE = "msm"
_RSS_URL    = "https://mustsharenews.com/feed/"
_BROWSER_UA = (
...
def scrape() -> list[dict]:
    ...
    try:
        ...
        if feed.bozo and not feed.entries:
            logger.warning("MustShareNews RSS parse error: %s", feed.bozo_exception)
            return results
        ...
        logger.error("MustShareNews scraper error: %s", exc)

    logger.info("MustShareNews: %d Yishun items", len(results))
    return results
...
if __name__ == "__main__":
    ...
    items = scrape()
    print(f"\nMustShareNews — {len(items)} Yishun item(s) found.")
    if items:
