"""
Straits Times scraper — RSS feed (spec: msm, 60-minute interval).
"""
...
SOURCE_TYPE = "msm"
_RSS_URL    = "https://www.straitstimes.com/news/singapore/rss.xml"
_BROWSER_UA = (
...
def scrape() -> list[dict]:
    ...
    try:
        ...
        if feed.bozo and not feed.entries:
            logger.warning("ST RSS parse error: %s", feed.bozo_exception)
            return results
        ...
        logger.error("Straits Times scraper error: %s", exc)

    logger.info("Straits Times: %d Yishun items", len(results))
    return results
...
if __name__ == "__main__":
    ...
    items = scrape()
    print(f"\nStraits Times — {len(items)} Yishun item(s) found.")
    if items:
