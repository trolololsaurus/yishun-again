...
# Minimum token ratio for keyword pre-filter before sending to Claude.
...
def check(candidate: dict, supabase_client=None) -> ConsolidationResult:
    ...
    candidate_kw   = _extract_keywords(candidate_text)

    candidates_to_judge: list[dict] = []
    for pub in published:
        pub_text = f"{pub['title']} {pub['summary']}"
        pub_kw   = _extract_keywords(pub_text)
        if _keyword_overlap(candidate_kw, pub_kw) >= _MIN_KEYWORD_OVERLAP:
    ...
    # ── Decision ─────────────────────────────────────────────────────────────
    if best_match and best_match_confidence >= 0.7:
        logger.info(
    ...
    if best_match and best_match_confidence >= 0.4:
