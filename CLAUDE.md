...
Map: MapLibre GL JS with OpenFreeMap "Liberty" style (`https://tiles.openfreemap.org/styles/liberty`). Keyless — no Mapbox token, no Stadia/CartoDB. Configured **only** via `NEXT_PUBLIC_MAPLIBRE_STYLE` — the hardcoded source fallback was removed (PR #3); if the env var is unset the map renders a "MAP UNAVAILABLE" state rather than silently using a default. The var **must** be set in Vercel (Production + Preview), and because `NEXT_PUBLIC_*` vars are baked at build time, changing it requires a fresh deploy, not just a restart.
